...
log = logging.getLogger('pymongodb')
...
class ConnectionWrapper(object):
	def __init__(self, connection, default=None, config={}):
	...
	def __getattr__(self, alias):
		...
		if self._config[alias]['authenticate']: 
			database.authenticate(self._config[alias]['username'], self._config[alias]['password'])		   
		log.info('pymongo initialized: %s', database)
		self._databases[alias] = database
		return database

	def __getitem__(self, alias):
		return self.__getattr__(alias)
